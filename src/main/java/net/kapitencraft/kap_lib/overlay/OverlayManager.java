...
import net.kapitencraft.kap_lib.overlay.box.ResizeBox;
...
public class OverlayManager {
     ...
     * order before INSTANCE is important as it is used inside the load() method
     */
    ...
    public static void setVisible(Overlay overlay, boolean b) {
        OverlayManager manager = INSTANCE;
        if (b) {
    ...
    private static @NotNull File getOrCreateFile() {
        if (PERSISTENT_FILE == null) {
            PERSISTENT_FILE = new File(LibConstants.ROOT, "overlay_config.json");
        }
    ...
    private void register() {
        ...
        ModLoader.postEvent(new RegisterConfigurableOverlaysEvent(this::createRenderer));
        construct();
    }
    ...
    public static void save() {
        IOHelper.saveFile(getOrCreateFile(), CODEC, INSTANCE);
    }
